...
class GaussianRenderer(Renderer):
    def __init__(self, num_parallel_scenes=16, viz=None):
        super().__init__()
    ...
    def _render_impl(
        ...
        save_ply_path=None,
        slider={},
        pose_estimation_training_data_widget_expanded=False,
        resolution_x=1024,
        resolution_y=1024,
        fov_x=45,
        fov_y=45,
        ...
        **other_args,
        ...
        for scene_index, ply_file_path in enumerate(ply_file_paths):
            ...
            # Render view of scene
            if pose_estimation_training_data_widget_expanded and scene_index == 1:
                cam_params = shift_camera_by_baseline(cam_params, baseline_mm, model_unit_mm)

            fov_rad_x = fov_x / 360 * 2 * np.pi
            fov_rad_y = fov_y / 360 * 2 * np.pi
            render_cam = CustomCam(
                # resolution_x, resolution_y, fovy=fov_rad_y, fovx=fov_rad_x, extr=cam_params_list[scene_index]
                resolution_x, resolution_y, fovy=fov_rad_y, fovx=fov_rad_x, extr=cam_params
            )
